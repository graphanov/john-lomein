#!/bin/bash
# Install one macOS system LaunchDaemon for the protected-action broker.
#
# SECURITY MODEL
# This is an operator-only installer. The script and broker source must first
# be staged in a root-owned tree. Running a sudo script from the requester-
# writable product checkout would let that requester replace privileged code
# while installation is in progress, so this script intentionally refuses it.
set -Eeuo pipefail
...
usage() {
  cat >&2 <<'EOF'
usage:
  install-protected-broker.sh \
    --slug SLUG \
    --config /absolute/root-owned/broker.json \
...
installer refuses to replace it while another broker instance is loaded.
EOF
  exit 2
}

die() {
  echo "protected broker install refused: $*" >&2
  exit 2
}
...
for required in \
  SLUG CONFIG_SOURCE GITHUB_KEY_SOURCE RECEIPT_PRIVATE_SOURCE \
  RECEIPT_PUBLIC_SOURCE PYTHON BROKER_USER REQUESTER_USER SUBMIT_GROUP
do
...
SOURCE_BROKER_DIR="$PRODUCT_ROOT/broker"

validate_existing_path "$SCRIPT_PATH" 0 file "installer script"
validate_root_owned_tree "$SOURCE_BROKER_DIR" "broker source"

validate_existing_path "$PYTHON" 0 executable "broker Python interpreter"
validate_existing_path "$CONFIG_SOURCE" 0 file "broker config source"
validate_existing_path \
  "$GITHUB_KEY_SOURCE" "0,$BROKER_UID" file "GitHub App key source"
validate_existing_path \
  "$RECEIPT_PRIVATE_SOURCE" "0,$BROKER_UID" file \
  "receipt private-key source"
validate_existing_path \
  "$RECEIPT_PUBLIC_SOURCE" "0,$BROKER_UID" file \
...
CONFIG_DIR='/private/etc/john-lomein-broker.d'
...
DATABASE_PATH="$STATE_DIR/broker.sqlite"
...
ENTRYPOINT="$CODE_ROOT/broker/run_broker.py"
LABEL="com.john-lomein.protected-broker.$SLUG"
PLIST_PATH="/Library/LaunchDaemons/$LABEL.plist"

TEMP_DIR="$(/usr/bin/mktemp -d /private/tmp/john-lomein-broker-install.XXXXXX)"
...
cleanup() {
  ...
  then
    ...
    if [ "$FILES_MUTATED" -eq 1 ]; then
      ...
      restore_managed_file \
        ...
        cleanup_status=1
      restore_managed_file \
        "$ROLLBACK_DIR/broker-config.json" "$CONFIG_PATH" ||
        cleanup_status=1
      restore_managed_file \
    ...
    if [ "$STATE_SNAPSHOTTED" -eq 1 ]; then
      if [ "$state_restore_safe" -eq 1 ]; then
        restore_managed_file \
          "$ROLLBACK_DIR/broker.sqlite" "$DATABASE_PATH" ||
          cleanup_status=1
        restore_managed_file \
          "$ROLLBACK_DIR/broker.sqlite-wal" "$DATABASE_PATH-wal" ||
          cleanup_status=1
        restore_managed_file \
          "$ROLLBACK_DIR/broker.sqlite-shm" "$DATABASE_PATH-shm" ||
          cleanup_status=1
        restore_managed_file \
          "$ROLLBACK_DIR/broker.sqlite-journal" "$DATABASE_PATH-journal" ||
          cleanup_status=1
      else
        cleanup_status=1
      fi
  ...
  if [ "$cleanup_status" -ne 0 ]; then
    echo \
      "protected broker install rollback was incomplete; service remains fail-closed" \
      >&2
    exit 70
  fi
  exit "$status"
...
snapshot_input() {
  ...
  local privacy="$4"
  local marker="$5"
  "$PYTHON" -I -S - "$source" "$destination" "$allowed_uids" \
    "$privacy" "$marker" <<'PY'
import os
import stat
import sys
...
CONFIG_SNAPSHOT="$TEMP_DIR/broker-config.json"
...
snapshot_input \
  "$GITHUB_KEY_SOURCE" "$GITHUB_KEY_SNAPSHOT" "0,$BROKER_UID" \
  private 'PRIVATE KEY'
snapshot_input \
  "$RECEIPT_PRIVATE_SOURCE" "$RECEIPT_PRIVATE_SNAPSHOT" \
  "0,$BROKER_UID" private 'PRIVATE KEY'
snapshot_input \
  "$RECEIPT_PUBLIC_SOURCE" "$RECEIPT_PUBLIC_SNAPSHOT" \
  "0,$BROKER_UID" public 'PUBLIC KEY'

ENABLED="$(
  "$PYTHON" -I - "$CONFIG_SNAPSHOT" "$GITHUB_KEY_SNAPSHOT" \
    "$RECEIPT_PRIVATE_SNAPSHOT" "$RECEIPT_PUBLIC_SNAPSHOT" \
    "$SLUG" "$BROKER_UID" "$REQUESTER_UID" "$SUBMIT_GID" \
    "$SOCKET_PATH" "$DATABASE_PATH" "$GITHUB_KEY_PATH" \
    "$RECEIPT_PRIVATE_PATH" "$RECEIPT_PUBLIC_PATH" <<'PY'
import hashlib
import json
import sys
from cryptography.hazmat.primitives import serialization
...
if len(raw) > 131072:
    raise SystemExit("broker config exceeds the installation size limit")
try:
    config = json.loads(
        raw,
    ...
    raise SystemExit(f"broker config is invalid JSON: {exc}") from exc

checks = (
    (config.get("schema_version") == "john-lomein.protected-broker-config.v1", "schema"),
    (config.get("instance", {}).get("slug") == slug, "instance slug"),
    (config.get("broker_uid") == int(broker_uid), "broker UID"),
    (
        config.get("transport", {}).get("requester_uid") == int(requester_uid),
        "requester UID",
    ),
    (
        config.get("transport", {}).get("submit_gid") == int(submit_gid),
        "submit GID",
    ),
    (
        config.get("transport", {}).get("socket_path") == socket_path,
        "socket path",
    ),
    (
        config.get("state", {}).get("database_path") == database_path,
        "database path",
    ),
    (
        config.get("github_app", {}).get("private_key_path")
        == expected_github_path,
        "GitHub private-key path",
    ),
    (
        config.get("receipt_signing", {}).get("private_key_path")
        == expected_receipt_private_path,
        "receipt private-key path",
    ),
    (
        config.get("receipt_signing", {}).get("public_key_path")
        == expected_receipt_public_path,
...
for passed, label in checks:
    if not passed:
        raise SystemExit(f"broker config {label} does not match installer binding")
if type(config.get("enabled")) is not bool:
    raise SystemExit("broker config enabled must be boolean")

github_private = serialization.load_pem_private_key(
    open(github_path, "rb").read(), password=None
)
...
receipt_private = serialization.load_pem_private_key(
    open(receipt_private_path, "rb").read(), password=None
)
receipt_public_bytes = open(receipt_public_path, "rb").read()
...
fingerprint = hashlib.sha256(receipt_public_bytes).hexdigest()
if config.get("receipt_signing", {}).get("public_key_sha256") != fingerprint:
    raise SystemExit("receipt public-key fingerprint does not match config")
print("1" if config["enabled"] else "0")
PY
)"
[ "$ENABLED" = 0 ] || [ "$ENABLED" = 1 ] ||
  die "broker config preflight returned an invalid enablement value"
...
ensure_secure_parent "$CODE_PARENT" "broker code parent"
...
ensure_managed_directory "$CONFIG_DIR" root "$BROKER_PRIMARY_GROUP" 0750 0 \
  "broker config directory"
ensure_managed_directory "$PUBLIC_DIR" root wheel 0755 0 \
  "broker public directory"
...
ensure_managed_directory \
  ...
  "instance broker run directory"
ensure_managed_directory \
  "$STATE_DIR" "$BROKER_USER" "$BROKER_PRIMARY_GROUP" 0700 "$BROKER_UID" \
  "instance broker state directory"
ensure_managed_directory \
  "$SECRETS_DIR" "$BROKER_USER" "$BROKER_PRIMARY_GROUP" 0700 "$BROKER_UID" \
  "instance broker secrets directory"
ensure_managed_directory \
  "$TMP_RUNTIME_DIR" "$BROKER_USER" "$BROKER_PRIMARY_GROUP" 0700 "$BROKER_UID" \
  "instance broker temporary directory"
...
backup_optional_file() {
  ...
  if [ -e "$source" ]; then
    ...
    "$PYTHON" -I -S - "$source" "$backup" "$allowed_uids" \
      "$maximum_bytes" <<'PY'
import os
import stat
import sys
...
try:
    ...
    try:
        copied = 0
        while True:
            chunk = os.read(source_fd, min(1024 * 1024, maximum + 1 - copied))
            if not chunk:
                break
            copied += len(chunk)
            if copied > maximum:
...
backup_optional_file \
  "$GITHUB_KEY_PATH" "$ROLLBACK_DIR/github-app.pem" "0,$BROKER_UID" \
  "existing installed GitHub App key"
backup_optional_file \
  "$RECEIPT_PRIVATE_PATH" "$ROLLBACK_DIR/receipt-private.pem" \
  "0,$BROKER_UID" "existing installed receipt private key"
backup_optional_file \
  "$RECEIPT_PUBLIC_PATH" "$ROLLBACK_DIR/receipt-public.pem" 0 \
  "existing installed receipt public key"
backup_optional_file \
  "$CONFIG_PATH" "$ROLLBACK_DIR/broker-config.json" 0 \
  "existing installed broker config"
backup_optional_file \
  "$CLIENT_CONFIG_PATH" "$ROLLBACK_DIR/client-config.json" 0 \
  "existing installed broker client config"
backup_optional_file \
  "$PLIST_PATH" "$ROLLBACK_DIR/launchdaemon.plist" 0 \
  "existing installed broker LaunchDaemon"

if /bin/launchctl print "system/$LABEL" >/dev/null 2>&1; then
  PREVIOUS_LOADED=1
  [ -f "$ROLLBACK_DIR/launchdaemon.plist" ] ||
    die "loaded broker has no safe LaunchDaemon plist to restore"
fi
...
for other_plist in "${OTHER_BROKER_PLISTS[@]}"; do
  ...
  if /bin/launchctl print "system/$other_label" >/dev/null 2>&1; then
    die \
      "shared broker code cannot be upgraded while another instance is loaded: $other_label"
  fi
done
...
stop_existing_service() {
  ...
  if /bin/launchctl print "system/$LABEL" >/dev/null 2>&1; then
    die "existing LaunchDaemon could not be verified stopped"
  fi
}
stop_existing_service
...
backup_optional_file \
  "$DATABASE_PATH" "$ROLLBACK_DIR/broker.sqlite" "0,$BROKER_UID" \
  "existing broker database" 8589934592
backup_optional_file \
  "$DATABASE_PATH-wal" "$ROLLBACK_DIR/broker.sqlite-wal" "0,$BROKER_UID" \
  "existing broker database WAL" 8589934592
backup_optional_file \
  "$DATABASE_PATH-shm" "$ROLLBACK_DIR/broker.sqlite-shm" "0,$BROKER_UID" \
  "existing broker database shared-memory file" 8589934592
backup_optional_file \
  "$DATABASE_PATH-journal" "$ROLLBACK_DIR/broker.sqlite-journal" \
  "0,$BROKER_UID" "existing broker database journal" 8589934592
STATE_SNAPSHOTTED=1
...
remove_stale_runtime_object "$SOCKET_PATH" socket "stale broker socket"
remove_stale_runtime_object "$SOCKET_LOCK_PATH" file "stale broker socket lock"

CODE_STAGE="$(
  /usr/bin/mktemp -d "$CODE_PARENT/.john-lomein-protected-broker.stage.XXXXXX"
)"
/usr/sbin/chown root:wheel "$CODE_STAGE"
/bin/chmod 0755 "$CODE_STAGE"
/usr/bin/install -d -o root -g wheel -m 0755 "$CODE_STAGE/broker"
shopt -s nullglob
BROKER_SOURCES=("$SOURCE_BROKER_DIR"/*.py)
shopt -u nullglob
[ "${#BROKER_SOURCES[@]}" -gt 0 ] || die "broker source has no Python modules"
for source_file in "${BROKER_SOURCES[@]}"; do
  [ -f "$source_file" ] && [ ! -L "$source_file" ] ||
    die "broker source module is unsafe"
  module_name="$(/usr/bin/basename "$source_file")"
  [[ "$module_name" =~ ^[A-Za-z0-9_]+\.py$ ]] ||
    die "broker source module name is unsafe"
  /usr/bin/install -o root -g wheel -m 0444 \
    "$source_file" "$CODE_STAGE/broker/$module_name"
done
[ -f "$CODE_STAGE/broker/run_broker.py" ] ||
  die "broker entrypoint is missing"
validate_root_owned_tree "$CODE_STAGE" "staged broker code"

if [ -e "$CODE_ROOT" ] || [ -L "$CODE_ROOT" ]; then
  validate_root_owned_tree "$CODE_ROOT" "existing installed broker code"
  CODE_BACKUP="$(
    /usr/bin/mktemp -d "$CODE_PARENT/.john-lomein-protected-broker.backup.XXXXXX"
  )"
...
CODE_STAGE=''
CODE_INSTALLED=1
validate_root_owned_tree "$CODE_ROOT" "installed broker code"
...
install_file_atomically \
  ...
  "$BROKER_PRIMARY_GROUP" 0600 "installed GitHub App private key"
install_file_atomically \
  "$RECEIPT_PRIVATE_SNAPSHOT" "$RECEIPT_PRIVATE_PATH" "$BROKER_USER" \
  "$BROKER_PRIMARY_GROUP" 0600 "installed receipt private key"
install_file_atomically \
  "$RECEIPT_PUBLIC_SNAPSHOT" "$RECEIPT_PUBLIC_PATH" root wheel 0444 \
  "installed receipt public key"
install_file_atomically \
  "$CONFIG_SNAPSHOT" "$CONFIG_PATH" root "$BROKER_PRIMARY_GROUP" 0640 \
  "installed broker config"

"$PYTHON" -I - "$CODE_ROOT" "$CONFIG_PATH" "$BROKER_UID" <<'PY'
import pathlib
import sys

code_root, config_path, broker_uid = sys.argv[1:]
sys.path.insert(0, code_root)
from broker.john_lomein_broker_protocol import load_config
from broker import john_lomein_broker_receipts
from broker import john_lomein_broker_service
from broker import john_lomein_github_app

load_config(
    pathlib.Path(config_path),
    expected_broker_uid=int(broker_uid),
)
PY

CLIENT_CONFIG_SNAPSHOT="$TEMP_DIR/client-config.json"
"$PYTHON" -I - "$CODE_ROOT" "$CONFIG_PATH" "$BROKER_UID" \
  "$CLIENT_CONFIG_SNAPSHOT" <<'PY'
import json
import pathlib
import sys

code_root, config_path, broker_uid, destination = sys.argv[1:]
sys.path.insert(0, code_root)
from broker.john_lomein_broker_protocol import config_digest, load_config
...
client = {
    ...
    "github_app_slug": github["app_slug"],
...
install_file_atomically \
  ...
  "installed broker client config"

PLIST_SNAPSHOT="$TEMP_DIR/$LABEL.plist"
"$PYTHON" -I - "$PLIST_SNAPSHOT" "$LABEL" "$PYTHON" "$ENTRYPOINT" \
...
payload = {
    ...
    "ThrottleInterval": 10,
    "Umask": 0o077,
    "StandardOutPath": f"{state_dir}/broker.stdout.log",
    "StandardErrorPath": f"{state_dir}/broker.stderr.log",
}
...
install_file_atomically \
  ...
  "installed broker LaunchDaemon"
/usr/bin/plutil -lint "$PLIST_PATH" >/dev/null

prepare_transaction_commit() {
  if [ -n "$CODE_BACKUP" ] && [ -d "$CODE_BACKUP" ]; then
    validate_root_owned_tree "$CODE_BACKUP" "broker code backup"
  fi
}
...
discard_transaction_backup() {
  if [ -n "$CODE_BACKUP" ] && [ -d "$CODE_BACKUP" ]; then
    /bin/rm -rf "$CODE_BACKUP" || {
      echo "warning: obsolete broker code backup remains at $CODE_BACKUP" >&2
    }
  fi
}
...
if [ "$ENABLED" -eq 0 ]; then
  ...
  if /bin/launchctl print "system/$LABEL" >/dev/null 2>&1; then
    die "disabled broker unexpectedly remained loaded"
  fi
  /usr/sbin/chown "$BROKER_USER:$BROKER_PRIMARY_GROUP" "$STATE_DIR"
  /bin/chmod 0700 "$STATE_DIR"
  STATE_DIR_QUARANTINED=0
  ...
  discard_transaction_backup
  echo "protected broker installed but disabled by config: $LABEL"
...
while [ "$attempt" -lt 50 ]; do
  ...
  /bin/launchctl print "system/$LABEL" >/dev/null 2>&1 ||
    die "enabled broker left launchd before opening its socket"
  /bin/sleep 0.2
  attempt=$((attempt + 1))
done
[ -S "$SOCKET_PATH" ] || die "enabled broker did not open its socket"
[ ! -L "$SOCKET_PATH" ] || die "broker socket is a symlink"
[ "$(/usr/bin/stat -f '%u' "$SOCKET_PATH")" -eq "$BROKER_UID" ] ||
  die "broker socket owner is incorrect"
[ "$(/usr/bin/stat -f '%g' "$SOCKET_PATH")" -eq "$SUBMIT_GID" ] ||
  die "broker socket group is incorrect"
[ "$(/usr/bin/stat -f '%Lp' "$SOCKET_PATH")" = 660 ] ||
  die "broker socket mode is not 0660"
reject_acl "$SOCKET_PATH" "broker socket"
/bin/launchctl print "system/$LABEL" >/dev/null 2>&1 ||
  die "enabled broker is not loaded"
...
TRANSACTION_COMMITTED=1
discard_transaction_backup
echo "protected broker installed and running: $LABEL"
