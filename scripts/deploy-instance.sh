...
if [ "${BOT_GUIDE_GATEWAY_ENABLED:-0}" = "1" ]; then
  "${PRODUCT_PYTHON[@]}" \
    ...
    --manifest "$JL_INSTANCE_MANIFEST_INPUT" >/dev/null
else
  "${PRODUCT_PYTHON[@]}" \
    "$BOT_HERMES_HOME/scripts/john_lomein_public_honcho_service.py" \
