...
BEGIN
  IF (SELECT count(*) FROM peers p WHERE p.workspace_name=current_setting('john_lomein.workspace') AND p.id IN (SELECT id FROM jl_peer_ids)
    ...
    OR (SELECT count(*) FROM messages WHERE workspace_name=current_setting('john_lomein.workspace') AND id IN (SELECT id FROM jl_message_ids))<>current_setting('john_lomein.expected_message_count')::bigint
    OR (SELECT count(*) FROM message_embeddings WHERE workspace_name=current_setting('john_lomein.workspace') AND id IN (SELECT id FROM jl_embedding_ids))<>current_setting('john_lomein.expected_embedding_count')::bigint
    ...
    OR (SELECT count(*) FROM queue WHERE workspace_name=current_setting('john_lomein.workspace') AND id IN (SELECT id FROM jl_queue_ids))<>current_setting('john_lomein.expected_queue_count')::bigint
  ...
  THEN
    RAISE EXCEPTION 'participant deletion exact-ID plan is stale';
  END IF;
END $$;
...
DELETE FROM queue WHERE workspace_name=:'workspace' AND id IN (SELECT id FROM jl_queue_ids);
DELETE FROM message_embeddings WHERE workspace_name=:'workspace' AND id IN (SELECT id FROM jl_embedding_ids);
...
DELETE FROM messages WHERE workspace_name=:'workspace' AND id IN (SELECT id FROM jl_message_ids);
