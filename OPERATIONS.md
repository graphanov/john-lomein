...
The one-command setup owns a fail-closed LaunchAgent lifecycle. It records labels per canonical instance manifest, binds each job to the expected runtime, exact command, and working directory, verifies `launchctl print` no longer sees a service before deleting its plist, and rolls back partially installed scheduler, keepawake, Guide, and dedicated public-Honcho jobs when a fatal setup or Doctor check fails. Disabling Guide removes the public-Honcho supervisor, including its API and deriver children. Use `make uninstall-supervisor INSTANCE=...` rather than deleting plists by hand so recorded labels from an earlier slug are reconciled too; personal `ai.hermes.honcho.*` services are outside this ownership set and are never stopped.
